...
23:21 的 attempt2 十小时等待审计中，更替后的两个外部 PID 已运行 8 小时 15 分钟，
A6000 为 6,570 MiB/95%，两个 compute process 仍常驻。3090/4090-2 继续处于
106/252 MiB、0% 的空闲基线，只读 SSHFS、隔离驱动、29668 端口及三机空输出
均正常；3070x2 自重启后稳定在线 11 小时 50 分钟。外部任务未修改，attempt2
与 `select12` 继续严格按序等待。

8 月 3 日 01:20 的 attempt2 十二小时等待审计中，更替后的两个外部 PID 已运行
10 小时 15 分钟，A6000 为 6,570 MiB/98%。3090/4090-2 仍为空闲基线，只读
SSHFS、隔离驱动、29668 端口及三机空输出均正常；3070x2 自重启后稳定在线
13 小时 47 分钟。外部任务未修改，attempt2 和 `select12` 继续按序等待。

03:20 的 attempt2 十四小时等待审计中，更替后的两个外部 PID 已运行 12 小时
15 分钟，A6000 为 6,570 MiB/99%。3090/4090-2 仍为空闲基线，只读 SSHFS、
隔离驱动、29668 端口及三机空输出均正常；3070x2 自重启后稳定在线 15 小时
48 分钟。外部任务未修改，attempt2 与 `select12` 继续严格按序等待。

05:20 的 attempt2 十六小时等待审计发现 PID 1500591 已在 04:50 至 05:20 间
自行退出，A6000 降至 2,169 MiB/41%，但 PID 1500488 仍占 2,144 MiB，平台
尚不干净。3090 首次状态命令完成但未捕获输出，带首尾 marker 的立即重试全部
...
12:56 attempt2 已完成 17/19 域并进入 `stargan`。72,373 张唯一图像均值
Acc/AP 为 95.4969%/97.3448%，相对匹配 baseline 为 +0.0292/+0.1443 点，
相对匹配 P1 为 +0.0477/+0.1805 点；17 域干净吞吐为 2.8849 images/s。
与 baseline 的索引、标签序列全部一致，共 260 个阈值分歧、加权概率 MAD
0.00391。原计划在 15 域归档，但复制期间又完成两域，已补拷 `seeingdark`
并重新生成一致的 17 域指标、对比和 profile。三机八 ranks、3090 只读 SSHFS、
4090-2 隔离驱动及 3070x2 数据源均健康，日志无错误。系统 Python 因缺少
...
17:53 select12 已完成前五域并进入 `gaugan` 的 100/1250 batch。26,811 张
唯一图像均值 Acc/AP 为 97.1380%/97.3498%，较匹配 select8 高
+0.1157/+0.1187 点，较匹配 P1 selection6 高 +0.1224/+0.2473 点。五域
索引和标签序列全部一致，相对两个参考分别有 120/171 个阈值分歧、加权概率
MAD 0.00475/0.00677。两个外部 A6000 训练任务仍在且未修改，最后复制样本的
raw host 显存为 43,613 MiB，保留此前 44,657 MiB 峰值。八 ranks、3090 只读
SSHFS 和三机日志均健康。预测指标及 rank-local 8,437.36/8,776 MiB 峰值有效，
...
21:22 select12 已完成 `ldm_200` 以前的 13 域并进入 `ldm_200_cfg`。15:58
启动的两个外部 A6000 任务在进程换代时退出，同命令、同父进程属主的新任务于
21:20:34/37 启动。raw host 显存仅在 21:20:38 至 21:20:49 的 12 个采样
短暂回到干净的 37,171 MiB select12 平台值，随后因新任务分配 CUDA 显存再次
升高。两代外部任务均未修改；该窗口位于域中段且过短，不能把 `ldm_200_cfg`
或累计耗时改判为干净数据。八 ranks、3090 只读 SSHFS 和三机日志均健康，预测
与 rank-local 显存继续有效。完整 A6000 曲线和进程换代审计已归档。首次远端
时间窗 `awk` 因嵌套 shell 中 `$1` 未转义而语法失败，改用时间戳查询后成功，
失败记录保留。

22:24 select12 已完成前 17 域并进入 `stargan`。72,373 张唯一图像均值
Acc/AP 为 95.5407%/97.4368%，较匹配 select8 高 +0.0438/+0.0920 点，
较匹配 P1 selection6 高 +0.0916/+0.2725 点。17 域索引和标签序列全部一致，
相对两个参考分别有 262/378 个阈值分歧、加权概率 MAD 0.00386/0.00553。
换代后的两个外部 A6000 训练任务仍在且未修改；八 ranks、3090 只读 SSHFS
和三机日志均健康。预测指标及 rank-local 8,437.36/8,776 MiB 峰值有效，
...
07:53 外部任务运行已达 10:32，仍占 2,088/4,380 MiB。A6000 瞬时利用率降到
...
11:23 同一组外部任务已运行 14:02，仍占 2,088/4,380 MiB，A6000 为
6,498 MiB/98%。其余两机空闲、3090 SSHFS 只读、steps3 三机输出为空。未修改
任何外部进程；为保护最后一个 P4 耗时点，`steps3` 与 P5 继续不启动。

13:24 发现外部任务已换代：旧 PID 最后在 12:53 存活，下一次检查时已退出；
同命令、同父进程属主的新 PID 1962366/1962431 已于 13:03:55/57 启动，各有
8 个子进程，占 2,088/4,396 MiB，A6000 为 6,512 MiB/90%。没有直接观测到
...
19:23 第二组 replacement 任务已运行 2:23。PID 1988891/1988960 占
4,400/2,148 MiB，A6000 为 6,576 MiB/99%；其余两机空闲，3090 SSHFS
保持只读，三机 `steps3` 输出仍为空。未修改外部进程，`steps3` 与 P5 均未
...
23:23 同一组外部 PID 已运行 6:23，仍占 4,400/2,150 MiB，A6000 为
...
03:23 同一组外部 PID 已运行 10:23，仍占 4,400/2,150 MiB，A6000 仍分配
6,578 MiB，瞬时利用率为 75%。其余两机空闲、3090 SSHFS 只读、三机
`steps3` 输出为空。外部进程未修改，`steps3` 与 P5 继续不启动。

05:23 同一组外部 PID 已运行 12:22，仍占 4,400/2,150 MiB，A6000 为
...
15:53 八小时干净快照已完成十二域，加入 `imle` 和 `ldm_100` 后均值为
95.2256% Acc / 97.0646% AP。相对 steps2 同域，Acc -0.0199 点、AP
-0.4257 点；相对 steps1，Acc +0.0178 点、AP -1.0697 点。全部 sampler
索引和标签序列一致。十二域聚合瓶颈延迟为 4,051.88 ms/image，是 steps2 的
1.486 倍、steps1 的 2.767 倍，rank-local 峰值分配仍为 8,437.55 MiB。
三机八 ranks、3090 只读 SSHFS 与 4090-2 隔离驱动均健康，当前运行
