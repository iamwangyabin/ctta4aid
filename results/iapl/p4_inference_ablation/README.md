...
The 21:21 eight-hour attempt2 wait audit sees both replacement external PIDs
still active after 6 hours 15 minutes, with A6000 at 6,570 MiB and 98%.
The 3090 and 4090-2 remain at their idle baselines, while the read-only SSHFS,
isolated driver, port 29668, and all empty output paths remain healthy. The
3070x2 source has stayed online for 9 hours 48 minutes since reboot. Neither
external job was modified; attempt2 and `select12` remain unlaunched.

The 23:21 ten-hour attempt2 wait audit sees both replacement external PIDs
still resident after 8 hours 15 minutes, with A6000 at 6,570 MiB and 95%.
The 3090 and 4090-2 remain idle at 106/252 MiB and 0%; the read-only SSHFS,
isolated driver, port 29668, and all empty output paths remain healthy. The
3070x2 source has stayed online for 11 hours 50 minutes since reboot. Neither
external job was modified; attempt2 and `select12` remain unlaunched in strict
order.

The 01:20 twelve-hour attempt2 wait audit sees both replacement external PIDs
still active after 10 hours 15 minutes, with A6000 at 6,570 MiB and 98%.
The 3090 and 4090-2 remain idle, while the read-only SSHFS, isolated driver,
port 29668, and all empty output paths remain healthy. The 3070x2 source has
stayed online for 13 hours 47 minutes since reboot. Neither external job was
modified; attempt2 and `select12` remain unlaunched in strict order.

The 03:20 fourteen-hour attempt2 wait audit sees both replacement external
PIDs still active after 12 hours 15 minutes, with A6000 at 6,570 MiB and 99%.
The 3090 and 4090-2 remain idle, while the read-only SSHFS, isolated driver,
port 29668, and all empty output paths remain healthy. The 3070x2 source has
stayed online for 15 hours 48 minutes since reboot. Neither external job was
modified; attempt2 and `select12` remain unlaunched in strict order.

The 05:20 sixteen-hour attempt2 wait audit finds that external PID 1500591
exited on its own between 04:50 and 05:20. A6000 fell to 2,169 MiB and 41%,
but PID 1500488 still holds 2,144 MiB, so the platform is not clean. The first
3090 status command completed without captured output; an immediate retry with
explicit markers passed all checks, and the transient failure is retained.
...
At 03:23 the same PIDs have run for 10:23 and still hold 4,400/2,150 MiB;
A6000 remains allocated at 6,578 MiB with a 75% instantaneous utilization
sample. The other hosts remain idle, the 3090 SSHFS is read-only, and steps3
output is absent everywhere. No external process was modified; steps3 and P5
remain unlaunched.

At 05:23 the same PIDs have run for 12:22 and still hold 4,400/2,150 MiB;
A6000 is 6,578 MiB/99%. The other hosts remain idle, the 3090 SSHFS is
read-only, and steps3 output is absent everywhere. No external process was
modified; steps3 and P5 remain unlaunched.
