...
class OnlineEvaluator:
    """Label-isolated evaluator for framework TTA methods.

    Most methods use Predict-Then-Adapt. Episodic methods may perform adaptation
    inside ``predict`` when adaptation is inseparable from their prediction
    contract. Hidden labels never enter either method call.
    """
    ...
    def run(
        ...
        *,
        ...
        for batch_index, batch in enumerate(stream):
            ...
            _sync_if_cuda(method.device)
            adapt_call_elapsed_ms = (time.perf_counter() - start) * 1000.0
            if adaptation.elapsed_ms is None:
                reported_predict_ms = predict_elapsed_ms
                reported_adapt_ms = adapt_call_elapsed_ms
            else:
                reported_adapt_ms = float(adaptation.elapsed_ms)
                reported_predict_ms = max(0.0, predict_elapsed_ms - reported_adapt_ms)
            ...
            batch_stats.append(
                {
                    ...
                    "predict_ms": reported_predict_ms,
                    "adapt_ms": reported_adapt_ms,
                    "total_ms": predict_elapsed_ms + adapt_call_elapsed_ms,
