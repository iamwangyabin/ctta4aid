...
3. **IAPL 补充轨道**：CLIP ViT-L/14、逐图 Adapt-Then-Predict 的独立方法能力；主表若列出 IAPL，必须标注其作者发布的任务 checkpoint，不能写成与未做任务训练的 zero-shot CLIP state 相同。
...
- CLIP-native 方法 TDA、DynaPrompt、CLIPTTA 与 BATCLIP 直接从相同预训练 checkpoint 出发，保留各自论文的文本分类器、template 或 prompt learner。它们只共享二分类类别语义，不共享一条人为固定的最终 prompt；不得使用目标标签选择文本或超参数。
- TDA 固定采用作者 clean/custom-dataset 路径的 `batch_size=1`：每个 online step 只交付一个 target 样本的一张确定性 global view，先更新正负 cache 再返回该样本预测。不得把多个不同 target 样本拼成 batch 后误当作同一图片的 augmentation views 做低熵筛选或特征平均。
- `frozen_clip` 的配置、实现和已验收结果继续保留，以便补充材料审计固定文本原型所包含的零样本语义信号。它没有任务特定的 detector head、源域训练或测试时更新，而且不是上述 CLIP-native 方法各自 prompt 构造的严格配对静态版本，因此不得进入正文定量表或参与最佳结果选择。
- IAPL 与 Ours 保留各自方法要求的源训练，但底层初始化仍必须来自同一固定 ViT-L/14 权重。正文必须披露其 method-specific source training 与其他方法的 source-state 差异；主表可以连续排列并作描述性的全局名次标记，但不得据此声称跨 source setup 的受控适应增益。
...
- Ours 的补充无边界复现流固定为 GenImage `SD1.5_first -> BigGAN -> ADM -> SD1.5_return`，正式 seed 为 `0/2/3`。每个 online episode 固定每类 224 个样本，每个 evaluator-only holdout episode 固定每类 112 个互斥样本；两个 SD1.5 episode 也必须彼此互斥。episode alias、generator identity、真实切换点和 labels 均不得传给方法。该补充只比较 `ours_static`、R37、R47、CoTTA 与 RoTTA-LN；不同方法可保留原生 batch size，因此 manifest 锁定样本身份和全局顺序而不锁定 delivery batch 划分。Detect、Route、Update 的因果消融必须复用同一 R47 类、同一 checkpoint、同一 manifest 和同一固定读出，每次只通过 `ablation_mode` 分别关闭一个模块，不得注册新的方法别名；消融结果写入对应 seed 下的 `ablations/no_detect`、`ablations/no_route` 和 `ablations/no_update`。完整流结果独立写入 `clip_vlm_bias_controlled/matched_jpeg/genimage_recurrence/seed<seed>`，不得混入正文单目标主表。
...
- 正文表格按一段连续的外部基线加一条横线后的 `Ours` 排列，不显示 source-setup 分组标题，也不在外部方法之间增加分隔。`ours_static` 只是冻结源状态对照，不得作为正文主方法行；它仅保留在读出消融、复现流和效率等明确的控制分析中，并在论文中标为 `Frozen source control` 而不是方法名。source setup 的差异必须在实验设置、metadata 和附录中完整披露。每列在全部正文行中按报告到小数点后两位的数值排序，最佳结果加粗、次佳的不同数值加下划线，同一报告精度下的并列结果共享相同标记；这种展示不得被解释为所有方法共享 source state，严格适应增益仍只能从共享起点的配对比较得出。
- 四个数据集必须分别生成逐 target 的 AUC 表和 Accuracy 表，不能只报告数据集级平均值。target 列及顺序固定为现有数据配置；Accuracy 使用阈值 0.5。每个 target 单元格报告三个正式 seed 的均值及跨 seed 标准差，标准差以较小下标显示；Mean 对每个 seed 先计算 target-macro，再报告三个 seed 的均值及标准差。
...
- `matched_jpeg` campaign 的正文单目标主实验只能写入 `clip_vlm_bias_controlled/matched_jpeg/<dataset>/seed<seed>`；上述固定无边界补充流只能写入 `clip_vlm_bias_controlled/matched_jpeg/genimage_recurrence/seed<seed>`。原始 `clip_vlm` campaign 保持不变并降为补充对照，不得写入、读取或补齐正文主实验目录。
