...
from online_aig_tta.data import build_domain_loader, concatenate_domain_streams
from online_aig_tta.data.streams import as_stream
from online_aig_tta.evaluation import (
    OnlineEvaluator,
...
def final_holdout_stream(config: dict, domains: list[str], seed: int):
...
def holdout_matrix_rows(checkpoints: list[dict]) -> list[dict]:
    rows = []
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/continual_stream.yaml")
    ...
    for method_name in config["methods"]:
        ...
        def evaluate_after_domain(current_method, completed_domain: str):
            ...
            holdout = evaluate_without_adaptation(
                current_method,
                final_holdout_stream(config, seen_domains, seed),
                threshold=threshold,
        ...
        result = evaluator.run(
            method,
            concatenate_domain_streams(config["data"], domains, seed=seed),
        ...
        result["holdout_matrix"] = holdout_matrix_rows(checkpoints)
